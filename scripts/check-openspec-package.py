...
REQ = re.compile(r'^### Requirement: ([A-Z]+-\d{3}) (.+)$', re.M)
SCENARIO = re.compile(r'^#### Scenario: ([A-Z]+-\d{3}-S\d+) (.+)$', re.M)
...
def check(root: Path, allow_progress: bool = False) -> dict:
    errors: list[dict] = []
    counts = dict(changes=0, capabilities=0, requirements=0, scenarios=0,
                  tasks=0, checked_tasks=0, legacy_tasks=0, legacy_acceptance=0)
    def error(code: str, path: object, detail: str):
        errors.append(dict(code=code, path=str(path), detail=detail))
    ...
    if not index or not trace:
        return dict(scope='limited-offline-document-check', passed=False,
    ...
    for pattern in (r'^schema: spec-driven$', r'^context: \|$', r'^rules:$'):
        if not re.search(pattern, config, re.M):
    ...
    for ch, definition in expected_changes.items():
        ...
        try:
            ...
            declared = set(re.findall(r'^- `([a-z0-9-]+)`', new_section, re.M))
        except IndexError:
            declared = set()
        ...
        for cn in sorted(actual):
            ...
            if re.search(r'^## (MODIFIED|REMOVED|RENAMED) Requirements',text,re.M):
            ...
            for i,m in enumerate(matches):
                rid,title=m.groups();counts['requirements']+=1
                if rid in seen_requirements: error('DUPLICATE_REQUIREMENT',path,rid)
                seen_requirements[rid]=dict(capability=cn,change=ch,spec=path)
                ...
                if len(re.findall(r'^#### Scenario:',block,re.M)) != len(sm):
                    error('SCENARIO_ID',path,rid+' invalid scenario id')
                for j,s in enumerate(sm):
                    sid,stitle=s.groups();counts['scenarios']+=1
                    ...
                    for kw in ('GIVEN','WHEN','THEN'):
                        if not re.search(r'^- \*\*'+kw+r'\*\* \S',sb,re.M):
    ...
    legacy_h=set(re.findall(r'^- \[ \] \*\*(H-\d+)\*\*',original,re.M))
    legacy_t=set(re.findall(r'^\|\s*(T-\d+)\s*\|',original,re.M))
    ...
    return dict(scope='limited-offline-document-check', passed=not errors,
                counts=counts, errors=errors, official_cli_executed=False,
                sdk_tests_executed=False,
                limitations=['Not the official OpenSpec parser or schema validator',
...
def main() -> int:
    ...
    try:
        ...
        result=dict(scope='limited-offline-document-check',passed=False,
                    errors=[dict(code='CHECK_ABORTED',path=str(args.root),detail=str(ex))],counts={},official_cli_executed=False)
