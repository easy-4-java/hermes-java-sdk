...
public class HermesClient implements AutoCloseable {
    ...
    private void checkHttpEnabled() {
        if (httpClient == null) {
            throw new IllegalStateException("HTTP client is disabled. Set HermesHttpClientConfig.enabled=true to enable.");
        }
    }

    private void checkCliEnabled() {
        if (cli == null) {
            throw new IllegalStateException("CLI client is disabled. Set HermesCliConfig.enabled=true to enable.");
        }
    }
