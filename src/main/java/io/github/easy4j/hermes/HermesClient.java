...
public class HermesClient implements AutoCloseable {
    ...
    private static void copyHttpConfig(HermesHttpClientConfig src, HermesHttpClientConfig target) {
        // 把测试用的 host 白名单旁路标志也复制过来——门面的内部 config
        // 在 copy 期间会被 setter 重新校验 URL，若漏掉此标志，MockWebServer 测试
        // 会因 baseUrl=127.0.0.1 而抛 IllegalArgumentException。
        ...
        target.setStreamMaxEventBytes(src.getStreamMaxEventBytes());
